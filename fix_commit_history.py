...
print("\n步骤 2: 修改 fix_all_private_keys.py...")
print("-" * 70)
...
stdout, stderr, code = run_cmd("git add fix_all_private_keys.py")
if code != 0:
    print(f"❌ add 失败: {stderr}")
    exit(1)

stdout, stderr, code = run_cmd('git commit --amend -m "Initial commit: Security configuration files (no real keys)"')
if code == 0:
    print("✅ 已修改初始提交")
else:
